...
test("artifact editor owns a complete shared modal focus contract", () => {
  ...
  assert.match(editor, /role="dialog"[\s\S]*aria-modal="true"[\s\S]*aria-label="会议产物工作区"[\s\S]*aria-busy=\{busy\}[\s\S]*tabIndex=\{-1\}/);
...
test("every artifact mutation makes all dismissal routes fail closed", () => {
  ...
  assert.match(editor, /const requestClose = \(\) => \{\s*if \(!busy\) onClose\(\)/);
  assert.match(editor, /onClose: busy \? null : requestClose/);
  ...
  assert.match(editor, /onClick=\{requestClose\} disabled=\{busy\}/);
...
test("every reopened artifact starts a fresh editor session before paint", () => {
  ...
  assert.match(dialog, /key=\{`\$\{renderedArtifact\.id\}:\$\{renderedArtifact\.version\}:\$\{editorSession\}`\}/);
});

test("focus hardening preserves the unique save confirm decision and export chains", () => {
  const editor = editorSource();

  assert.equal((editor.match(/await onSave\(currentArtifact, \{ keepOpen: true \}\)/g) || []).length, 1);
  assert.equal((editor.match(/await onSave\(currentArtifact\)/g) || []).length, 1);
  assert.equal((editor.match(/await onConfirm\(currentArtifact\)/g) || []).length, 1);
  assert.equal((editor.match(/onSubmit=\{onUserDecision\}/g) || []).length, 1);
  assert.equal((editor.match(/onExport\(currentArtifact\)/g) || []).length, 1);
});

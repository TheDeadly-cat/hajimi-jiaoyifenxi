...
test("material focus ownership follows the real editor or attestation surface without closing the modal lifecycle", () => {
  ...
  assert.match(source, /useLayoutEffect\(\(\) => \{[\s\S]*initializedMaterialRef\.current === material[\s\S]*setInitializedMaterial\(material\)/);
  assert.match(source, /const materialSurfaceOpen = Boolean\(open && form && initializedMaterial === material\)/);
...
test("material and attestation busy states fail closed for every dismiss and submit route", () => {
  ...
  assert.match(source, /const run = async \(action, onSuccess\) => \{\s*if \(busy\) return null/);
  ...
  assert.match(source, /event\.preventDefault\(\);\s*if \(!busy\) submitPrimary\(\)/);
...
test("focus hardening preserves material and server-hash confirmation payload paths", () => {
  ...
  assert.match(source, /if \(mode === "url"\) return run\(\(\) => onFetchUrl\(form\)\)/);
  assert.match(source, /\(\) => onImportFile\(form, file\)/);
  assert.match(source, /onSubmit\(\{ \.\.\.form, kind: "note", source_url: "" \}\)/);
  assert.match(source, /const saveManualRevision = \(\) => run\(\(\) => onSubmit\(form\)\)/);

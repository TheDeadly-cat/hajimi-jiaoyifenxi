...
阶段 4 只复用现有 `FutuUsMarketAdapter.quote_batch()`，一次读取固定四股
...
`build_futu_anomaly_registry()`。生产 builder 只接受字面量回环 OpenD 地址和现有
`FutuUsMarketAdapter` 的精确只读 `quote_batch` callable。构造 registry 不导入 Futu
SDK、不探测 socket、也不读取行情。

...
功能提交 `e2e5d2ddc90481272d8dc9c396344c910d7d6bf6` 的 push run `35476896150` 于 2026-09-20 08:40 北京时间结束：2,269 项、3193.388 秒、1 失败、1 错误。此前五类路由/候选到期失败没有再出现，但该次完整 CI 仍失败，后续干净源码和安装升级回滚检查被跳过。

剩余两项为停止线程测试中 `wait_until_stopped(2)` 返回 False，以及只读 MCP 拒绝错误 Origin 时发生 Windows `ConnectionAbortedError / 10053`。本机首次分别运行两项均通过；随后各重复 20 次，40 次执行中复现 1 次同样的 MCP 错误，停止线程测试 20 次均通过。没有据此宣称 CI 的线程等待失败已修复，也没有放宽其等待或安全断言。

MCP 处理器原先在读取请求正文前拒绝 Origin 并关闭连接。客户端分开发送 HTTP 头和正文时，未读字节可能导致关闭连接时丢失拒绝响应。新增分段发送回归在修复前确定失败。修复后，仅对 `/mcp` 的合法 Content-Length 帧先读取完整正文，沿用 64 KiB 上限，并增加 5 秒单调时钟总读取窗口，然后执行原有 Origin、Accept、类型、协议和授权检查。超时返回 408，未进入应用处理；不重试请求、不放宽 Origin 或授权。超大/无效帧仍直接拒绝，不承诺为任意恶意帧保留响应。

最终定向验证：两个模块 39 项，加同一 MCP 场景额外重复 20 次，共 **59 次执行、39 个不同测试、零失败/错误/跳过，50.673 秒**；网络审计为随机回环连接 91、外网/子进程阻断尝试均 0。包含分段正文的 403 返回，以及未完整发送正文时的有界 408 返回；两者均证明不调用应用处理。静态检查 7/7。上述数据不与此前 226 项重叠相加，也不替代新提交的完整远端终态。

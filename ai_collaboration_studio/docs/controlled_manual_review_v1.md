...
保留所有首轮记录，包括旧候选 CI 的 2,243 项 / 27 失败 / 3 错误、本机五类代表性失败、首次夹具修正结果、新增到期测试对 409 的错误预期，以及新 CLI 初始化顺序错误。测试修正没有削弱生产防篡改、预算或候选到期判断；没有把生产核验日期改成今天。

总数更正：最初说明沿用了任务书引述的 2,236 项 / 3030.079 秒。直接下载 run `35471982969`、job `105974476455`、attempt 1 的终态日志后，核实同一 `4d2bf623` 候选的实际记录为 `Ran 2243 tests in 3037.262s`，失败/错误数仍为 27/3。原引述及原日志均保留，不将引用差异视为不同的测试覆盖或额外失败。

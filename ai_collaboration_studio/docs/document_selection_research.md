...
在目标房间打开「ChatGPT 协作」；若已有冻结任务，先点「创建新任务」（已有过期流程可能显示「生成新任务包」），旧任务保留在历史列表。然后点「预览实际研究资料」，展开资料核对完整摘录后才能冻结新任务包。该步只产生 BUNDLE_READY，不发送到 ChatGPT、不运行 Provider/API 审查或正式 round。旧「附加到房间」仍附加原事件，旧 round-draft 流程不变。
...
完整提示另受现有前端 500,000 UTF-16 code units 导出上限约束。新冻结在写入任何任务记录前按同一计数拒绝超限；每份材料小于 1,600 字符并不代表复杂候选/历史上下文生成的整包一定可导出。合成复现得到 510,424 单位的提示，所选证据位于 504,297 之后，旧前端切片确实会丢掉它；本次用拒绝门保护完整选段，没有提高导出预算或改写旧冻结包。

最终运行修复版本 `2a84b45a4e4c5f333a14c625aa7df4eebc109cc6` 的独立离线 Edge 验收：已有房间通过“创建新任务 → 预览实际研究资料 → 冻结任务包”得到新的 `BUNDLE_READY`。两份历史 Micron 选段共 2,481 字符（1,448 + 1,033），全文提示 10,788 UTF-16 单位；材料、来源 URL 与冻结摘录逐字符一致，两份旧冻结包不变。此轮浏览器无脚本、控制台、HTTP 错误或外部请求。此前健康快照读取的 409 原始失败记录仍保留，最后一次成功不能证明该问题已修复。

总导出边界修复后运行的定向结果为 38 项后端、2 文件 / 18 项前端测试通过，生产构建通过。后端记录 18 次允许的回环连接，外网和子进程外网尝试为零。这些与此前 92 / 62 等覆盖组存在重叠，不累加为独立覆盖；远端 CI 以最终提交对应的终态回执为准。
...
补查发现：既有 BUNDLE_READY 页面缺少「创建新任务」，会阻止在同一房间使用后加入的资料。补上独立新任务入口与旧包不自动纳入新资料的提示；没有修改旧任务状态。新增回归先复现缺按钮，再通过；相关前端 2 文件 18 项通过。运行 `61eafbb4ddf31734ca7e81a69a5529cf70ae6b34` 后，在已有冻结任务的同一房间追加第 2 段、重新预览两份材料并冻结新包，两份 excerpt 均与材料逐字相同，旧包重新读取保持原样。

该补查的严格“控制台必须为空”断言未通过：快速打开收件箱时既有 `/api/monitoring/health` 两次返回 409 / SOURCE_MONITORING_HEALTH_READ_FAILED（快照复制临时失败）；单独本地重读返回 200。选段与冻结接口均正常，未产生脚本异常或外网/模型调用。保留为非阻断健康状态读取问题，根因未进一步定位，本轮不声称已修复，也不将其记成“全部控制台无错误”。

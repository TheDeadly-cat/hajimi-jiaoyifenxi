...
- 官方来源监控现由 `SourceMonitoringRuntime v1` 接入宿主生命周期：默认仍关闭、默认不自动启动、默认 dry-run 与 `seed_only`；只有全局 enabled + auto-start 且 adapter 已被显式启用时，单个受管线程才串行执行。首次模式支持 sealed `seed_only/catch_up/from_time`，catch-up 必须绑定预览哈希；本地 CLI 提供只读 `status/preview` 与显式确认的 `run-once`，不开放 run-now HTTP。`source_monitoring_health_service_v2` 以实时线程和 monotonic heartbeat 区分 stopped/running/degraded/stalled/failed，持久化 RUNNING 行不再伪装在线。Runtime、CLI、初始化收据、`retain_all_evidence`、正式迁移与回滚边界见 [`docs/source_monitoring_operations_runbook.md`](docs/source_monitoring_operations_runbook.md)。
